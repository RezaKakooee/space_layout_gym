...
def get_learner_config(env, env_name, agent_config, config):
    config.update({"framework": agent_config['framework'],
                   "env": env_name,
    ...
    if agent_config['some_agents'] == 'Multi':
       ...
       config.update({"multiagent": {"policies": my_policy.define_policy(),
    ...
    if env_name == 'master_env-v0':
        if  env.fenv_config['net_arch'] == 'cnn':
            config.update({"model": {"dim": 21,
                                     "conv_filters": [
                                         [16, [3, 3], 1],
                                         [32, [5, 5], 2],
                                         [512, [11, 11], 1]
    ...
    if agent_config['custom_model_flag']:
        if env.env_name == "pistonball_v4":
            ...
            if env.fenv_config['net_arch'] == 'cnnfc':
                config.update({"model": {
                                         "dim": 21,
                                          "conv_filters": [
                                                           [16, [3, 3], 1],
                                         ...
                                         "custom_model": "cnnfc_model",
                                         "vf_share_layers": True,

                                         "custom_model_config": {"conv_filters": [21, 21, 1]},
                                         }
                               })
            elif env.fenv_config['net_arch'] == 'fccnn':
                config.update({"model": {
                                         "dim": 21,
                                          "conv_filters": [
                                                           [16, [3, 3], 1],
